...
export default {
    ...
    axios.post('api/dashboard/tickets/' + self.$route.params.uuid + '/reply', self.ticketReply).then(function (response) {
        ...
        self.ticket = response.data.ticket; // Mengupdate tiket
        self.ticketReply.status_id = response.data.ticket.status_id;
        self.discardReply(); // Mengatur ulang form balasan
        self.loading.reply = false;

        // Navigasi setelah membalas pesan
        self.$router.push('/dashboard/tickets'); // Arahkan ke halaman tiket
    }).catch(function () {
        self.loading.reply = false;
    });
},
